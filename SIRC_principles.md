...
*v3.10 · Retraction log: [SIRC_principles_retraction.md](SIRC_principles_retraction.md)*
...
| § $\mathsf{P2}$ — Entropy | Across a finite discrete interface between heterogeneous substrates, full-state transmission is lossy by necessity. The protocol targets invariant preservation rather than full fidelity, because full fidelity is unachievable and invariant preservation is the required validity threshold ( $\mathsf{P1}$). |
...
The constraints span four mathematical fields by design. The transmission problem requires independent constraints from four domains: validity (formal logic), limits (information theory), encoding (algorithmic information theory), and cost (computational complexity). Categorical diversity across the set is by design. Each constraint is necessary — removing any one loses an independent dimension of the transmission problem. None is derivable from the others. "Necessary" here denotes dimensional independence — each domain supplies a structural constraint not derivable from the other three. It does not claim each domain is the unique proof route to its conclusion; alternative mathematical routes to individual findings may exist. The claim is that the set cannot be reduced without losing an independent structural dimension: information theory contributes the loss bound (what is unrecoverable at the floor); no other domain in the set characterises recovery limits under finite encoding.
...
**Enumerable claims** — values that a verifier can compute from a fully specified structure: $|\mathcal{B}_-|$, $|\mathcal{B}_+|$, $N_{paths}$, $L_{\min}$, edge counts, ablation outcomes. These are proved by computation. An UNSAT certificate from Z3 BMC is a machine-checkable proof. Any reader can reproduce it independently. Multi-model audit adds nothing here — the verifier is authoritative.
...
## § $\mathsf{P2}$ — Entropy
Across a finite discrete interface between heterogeneous substrates, full-state transmission is lossy by necessity. Whether invariant structure is also at risk under substrate mismatch is unresolved (OQ2.1). The protocol targets invariant preservation rather than full fidelity, because full fidelity is unachievable and invariant preservation is the required validity threshold ( $\mathsf{P1}$).

### Grounding

- **Information theory** --- lossy source coding and the data processing inequality, applied as a two-stage chain:
  - *Argument (continuous-source rate bottleneck):* Full-state fidelity is unachievable when a continuous-source state is encoded into a finite discrete packet. Under substrate heterogeneity — where a continuous high-dimensional neural activation state $S_{\text{send}} \in \mathbb{R}^n$ must be transmitted via a finite discrete constraint packet $Y \in \Sigma^*$ — achieving zero distortion requires infinite rate: $R(0) = \lim_{D \to 0} R(D) = \infty$. Because packet length is strictly bounded ($|Y| < \infty$), the mutual information is strictly bounded ($I(S_{\text{send}}; Y) \le H(Y) < \infty$), guaranteeing non-zero distortion ($D > 0$). The necessity of loss follows from the continuous source alone — no ad-hoc distortion metric required.
  - *Argument (discrete-source bottleneck):* For discrete heterogeneous sources, full-state loss is grounded in the finite-channel bottleneck, not in rate-distortion continuity. The $R(0) = \infty$ argument assumes $S_{\text{send}} \in \mathbb{R}^n$ and does not apply when the source is itself discrete (e.g. prover-to-prover between heterogeneous symbolic systems). Loss is instead grounded in the finite-channel condition: source entropy exceeding channel capacity forces $I(S_{\text{send}}; Y) \le C < H(S_{\text{send}})$, guaranteeing equivocation $H(S_{\text{send}} \mid Y) > 0$.
  - *Argument (irrecoverability — DPI):* DPI establishes that the encoding loss is permanent. For the Markov chain $S_{\text{send}} \to Y \to S_{\text{recv}}$, the data processing inequality dictates that $I(S_{\text{send}}; S_{\text{recv}}) \le I(S_{\text{send}}; Y)$. No downstream computation, search heuristic, or receiver prior can recover information about $S_{\text{send}}$ excluded from $Y$.
  - *Scope (substrate condition):* The necessity claim is conditioned on heterogeneous representational spaces transmitting across a finite discrete interface or constrained channel — where source entropy exceeds channel capacity (following the usage in § $\mathsf{P4}$: "constrained channel capacity"). Between homogeneous substrates with an unconstrained channel, full-state transmission is achievable and $\mathsf{P2}$ does not bind; the protocol's transmission problem does not arise.
  - *Scope (invariant vs. full state):* $\mathsf{P2}$ applies to the full substrate state $S_{\text{send}}$, not to the invariant projection $\mathcal{I}(S_{\text{send}})$. The invariant is designed to satisfy $H(\mathcal{I}(S_{\text{send}}) \mid Y) = 0$ — full invariant recovery is the protocol target ( $\mathsf{P1}$). Whether invariant content is also at risk under substrate mismatch is OQ2.1, not a consequence of source coding or DPI directly.
  - *Scope (metric status):* Continuous source coding establishes the qualitative necessity of loss ($D > 0$). Quantifying the distortion-rate curve over specific surface embeddings requires an empirical distortion metric that SIRC does not currently provide — that quantification is OQ2.1's domain. The existence of the bottleneck is grounding; its geometric distribution across surface features is open.
...
- *Argument (why invariance, not fidelity):* $\mathsf{P2}$ is the reason $\mathsf{P1}$ targets invariant preservation — full fidelity is unachievable across a heterogeneous or capacity-limited interface (lossy source coding) and unrecoverable downstream (data processing inequality), so the system preserves what is P1-invariant and accepts surface loss.
...
- OQ2.1 — The loss profile under substrate mismatch has two distinct components that must not be conflated:
  ...
  - *Scope (expressive incommensurability):* A distinct degradation condition arises when the receiver's logic is expressively too weak to represent the sender's operator types — e.g. a higher-order or dependently-typed source reconstructed in a first-order or Horn-clause receiver. This loss is not information-theoretic: it persists at unbounded channel capacity ($C = \infty$) and does not reduce to continuous-source quantization or the finite-channel bottleneck of § $\mathsf{P2}$. It is also distinct from OQ1.4 (different inference rules over the same representable structure) — here the structure is not representable in the receiver's logic at all. The Preamble substrate scope handles the total case (a receiver incapable of the required operators is out of scope); the graded case — partial representability with expressive-tier collapse — is the open region, with abstract model theory / institution theory (theory doc) the candidate grounding direction, since Shannon has no expressive-power content. Whether this graded loss is a $\mathsf{P2}$ loss mode requiring a non-information-theoretic grounding or a $\mathsf{P1}$ scope boundary is the open verdict this sub-item hands the receiver.
...
- OQ3.1 — What constitutes the minimum sufficient boundary conditions for a reasoning structure is not yet formally characterised. The existence of such a minimum follows from $MDL$; the procedure for finding it does not.
  ...
  - *Argument (historical negative space):* *(C)* the computational criterion above applies to current-state formats — what the artifact IS. A living artifact that evolves through successive revisions requires a second axis: what the artifact has excluded and why. A retraction log carries information not recoverable from any current-state document — closed exploration paths, the causal reasons behind current constraints, and anti-convergence records that prevent a future receiver from re-proposing exhausted search space. This is $\mathcal{B}_-$ across time rather than $\mathcal{B}_-$ across the current concept boundary $\mathcal{B}_-$ ([defined in SIRC_glossary.md](SIRC_glossary.md)). Current-state formats alone cannot reconstruct why the artifact's boundary is where it is — the minimum sufficient format set for a living artifact therefore includes at least one format covering historical negative space.
